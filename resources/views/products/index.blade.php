                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="flex flex-wrap gap-1 mt-1.5">
                                    @if($product->total_sold > 50)
                                        <span class="inline-flex items-center rounded-md bg-orange-50 px-1.5 py-0.5 text-[10px] font-medium text-orange-700 border border-orange-100">🔥 Terlaris</span>
                                    @endif
                                    @if($product->created_at && $product->created_at->diffInDays(now()) <= 7)
                                        <span class="inline-flex items-center rounded-md bg-sky-50 px-1.5 py-0.5 text-[10px] font-medium text-sky-700 border border-sky-100">✨ Baru</span>
                                    @endif
                                    @if($product->stock > 100)
                                        <span class="inline-flex items-center rounded-md bg-purple-50 px-1.5 py-0.5 text-[10px] font-medium text-purple-700 border border-purple-100">📦 Overstock</span>
                                    @endif
                                    @if($product->cost_price && ($product->price - $product->cost_price) > ($product->cost_price * 0.5))
                                        <span class="inline-flex items-center rounded-md bg-emerald-50 px-1.5 py-0.5 text-[10px] font-medium text-emerald-700 border border-emerald-100">💸 Profit Tinggi</span>
                                    @endif
                                </div>
                            </div>
                        </td>
