...
<div class="mb-8 animate-fade-in-up">
    ...
    <h1 class="mt-2 text-3xl font-extrabold text-gray-900 tracking-tight">Edit Produk: {{ $product->name }}</h1>
</div>
        ...
        <form action="{{ route('products.update', $product) }}" method="POST">
            ...
            <div class="mb-6">
                <label for="cost_price" class="block text-sm font-semibold text-gray-700 mb-2">
                    Harga Modal (Rp) <span class="text-gray-400 font-normal">(Opsional)</span>
                </label>
                <input type="number"
                       id="cost_price"
                       name="cost_price"
                       ...
                       class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-900
                              focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all duration-200
                              @error('cost_price') border-red-300 @enderror">
                @error('cost_price')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
